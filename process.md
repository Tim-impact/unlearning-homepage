...
**마지막 업데이트:** 2026-06-26 (Work 헤드라인 모바일 줄바꿈 정리 — clamp 바닥값 ↓)
...
- [x] **Work 헤드라인 모바일 줄바꿈 정리 (B안)** — "성장을 위한 더 큰 도전을 함께 하는 / AX 오퍼레이션 파트너" 헤드라인이 좁은 화면에서 1행이 넘쳐 3줄이 되던 문제. h2 폰트 `clamp` 바닥값을 26px→18px로 낮춰(`clamp(18px, 4vw, 38px)`) 폭 650px 미만(폰)에서 1행("성장을…하는")이 줄어들며 들어맞게 함. 단, 초록 키워드 "AX 오퍼레이션 파트너"는 span에 원래 값(`clamp(26px, 4vw, 38px)`)을 명시해 **이전 크기 유지**(모바일 강조 보존). 폭 650px 이상에선 두 값이 같아져 **태블릿·노트북·데스크톱은 동일**. 강제 `<br/>` + 초록 2행 단독 구조는 의도대로 보존 (2026-06-26)
